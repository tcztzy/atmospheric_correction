...
from zac.reproject import array_to_raster, reproject_data
...
def warp_data(fname, aoi, xRes, yRes):
    g = gdal.Warp(
        ...
        cropToCutline=True,
        resampleAlg=0,
    )  # weird adaptation for gdal 2.3, this should be a bug in gdal 2.3
    return (
        g.ReadAsArray()
...
def convolve(img, gaus_2d, hx, hy):
    ...
    dat = idct(
        idct(
            dct(dct(img, axis=0, norm="ortho"), axis=1, norm="ortho") * gaus_2d,
            axis=1,
            norm="ortho",
        ),
        axis=0,
...
def fill_nan(array):
    ...
    value = array[mask]
    mesh = (
        np.repeat(range(x_shp), y_shp).reshape(x_shp, y_shp),
...
class solve_aerosol:
    ...
    def __init__(
        ...
        try:
            ...
            self.spec_map = Two_NN(
                np_model_file=spec_m_dir + "/Aqua_%s_spectral_mapping.npz" % self.sensor
            )
    ...
    def _get_psf(self,):
        ...
        self.logger.info(
            "Solved PSF: %.02f, %.02f, %d, %d, %d, and R value is: %.03f."
        ...
        self.hx, self.hy = (
            self.hx[shifted_mask] + int(xs),
            self.hy[shifted_mask] + int(ys),
        )
    ...
    def _mask_bad_pix(self):
        ...
        if 3 in self.boa_bands:
            BLUE = (
                self._toa_bands[np.where(self.boa_bands == 3)[0][0]].ReadAsArray()
                * self.ref_scale
                + self.ref_off
            )
        if BLUE is not None:
        ...
        if 2 in self.boa_bands:
            NIR = (
                self._toa_bands[np.where(self.boa_bands == 2)[0][0]].ReadAsArray()
                * self.ref_scale
                + self.ref_off
            )
        if 1 in self.boa_bands:
            RED = (
                self._toa_bands[np.where(self.boa_bands == 1)[0][0]].ReadAsArray()
                * self.ref_scale
                + self.ref_off
            )
        ...
        if 6 in self.boa_bands:
            SWIR_1 = (
                self._toa_bands[np.where(self.boa_bands == 6)[0][0]].ReadAsArray()
                * self.ref_scale
                + self.ref_off
            )
        if 1 in self.boa_bands:
            GREEN = (
                self._toa_bands[np.where(self.boa_bands == 4)[0][0]].ReadAsArray()
                * self.ref_scale
                + self.ref_off
            )
        ...
        mask = binary_erosion(
            mask, structure=np.ones((3, 3)).astype(bool), iterations=5
        ).astype(bool)
        # mask     = binary_dilation(mask, structure = np.ones((3,3)).astype(bool), iterations=30).astype(bool)
        mask = binary_dilation(
            mask, structure=np.ones((3, 3)).astype(bool), iterations=30 + ker_size
        ).astype(bool)
    ...
    def _create_band_gs(self,):
        ...
        self.xSize = int(np.ceil(x_max_pix / (1.0 * self.aero_res)))
        self.ySize = int(np.ceil(y_max_pix / (1.0 * self.aero_res)))
    ...
    def _resamplers(self,):
        ...
        """
        self.nearest_resampler = lambda fname: reproject_data(
            fname,
            self.example_file,
            ...
            outputType=gdal.GDT_Float32,
            resample=0,
        ).g  # GRIORA_NearestNeighbour due to version changes...

        self.bilinear_resampler = lambda fname: reproject_data(
            fname,
            self.example_file,
            xRes=self.aero_res * 0.5,
            yRes=self.aero_res * 0.5,
    ...
    def _read_aux(self,):
        ...
        """
        self._ele = (
            self.bilinear_resampler(self.global_dem).ReadAsArray() * self.ele_scale
        )

        time_ind = np.abs(
            (
                self.obs_time.hour
                + self.obs_time.minute / 60.0
                + self.obs_time.second / 3600.0
            )
            - np.arange(0, 25, 3)
        ).argmin()
        ...
        for i in range(3):
            if priors[i] is None:
                priors[i] = self.cams_dir + "/".join(
                    [
                        datetime.strftime(self.obs_time, "%Y_%m_%d"),
                        datetime.strftime(self.obs_time, "%Y_%m_%d")
                        + "_%s.tif" % cams_names[i],
                    ]
                )
                use_cams[i] = True
        ...
        for _, i in enumerate(priors):
            ...
            if use_cams[_]:
                ...
                scale = g.GetScale()
                data = (
                    prior_g.GetRasterBand(int(time_ind + 1)).ReadAsArray() * scale
                    + offset
                )
            else:
    ...
    def _find_boa_bands(self,):
        ...
        self.boa_bands = np.array([1, 2, 3, 4, 6, 7])[
            band_index,
        ]
        self.boa_wv = self.boa_wv[
            band_index,
        ]
        self.toa_bands = self.toa_bands[
            mask,
        ]

        order = np.argsort(self.band_wv[mask,])
        self.boa_bands = self.boa_bands[
            order,
        ]
        self.boa_wv = self.boa_wv[
            order,
        ...
        if sa_va_seperate is True:
            ...
            self.view_angles = self.view_angles[
                mask,
            ]
            self.view_angles = self.view_angles[
                order,
    ...
    def _read_MCD43(self, fnames):
        par = partial(
            warp_data, aoi=self.aoi, xRes=self.aero_res * 0.5, yRes=self.aero_res * 0.5
        )
        p = Pool()
        p = Pool(procs)
    ...
    def _get_boa(self, temporal_filling=16):
        ...
        if temporal_filling:
            ...
            for i in range(No_band):
                ...
                chunks = zip(
                    np.array_split(das, 18, axis=1), np.array_split(Ws, 18, axis=1)
                )
    ...
    def _get_index(self, mg, hg):
        ...
        xgeo = (
            geotransform[0] + np.arange(0.5, mg.RasterXSize + 0.5, 1) * geotransform[1]
        )
        ygeo = (
            geotransform[3] + np.arange(0.5, mg.RasterYSize + 0.5, 1) * geotransform[5]
        )
        ...
        h_xgeo, h_ygeo, _ = np.array(
            osr.CoordinateTransformation(m_proj, h_proj).TransformPoints(
                list(zip(xgeo[temp_data], ygeo[temp_data]))
            )
        ).T
    ...
    def _load_xa_xb_xc_emus(self,):
        ...
        for band in self.toa_bands:
            ...
            xap_emu = glob(
                self.emus_dir
                + "/isotropic_%s_%s_%s_xap.npz"
                % (self.sensor, self.satellite, band_name)
            )[0]
            xbp_emu = glob(
                self.emus_dir
                + "/isotropic_%s_%s_%s_xbp.npz"
                % (self.sensor, self.satellite, band_name)
            )[0]
            xcp_emu = glob(
                self.emus_dir
                + "/isotropic_%s_%s_%s_xcp.npz"
                % (self.sensor, self.satellite, band_name)
            )[0]
    ...
    def _get_convolved_toa(self,):
        ...
        if self.full_res[0] % 2 != 0:
            xgaus = np.exp(
                ...
                * (self.psf_xstd ** 2)
                * (
                    (0.5 * np.arange(self.full_res[0] + 1) / (self.full_res[0] + 1))
                    ** 2
                )
            )
        else:
        ...
        if self.full_res[1] % 2 != 0:
            ygaus = np.exp(
                ...
                * (self.psf_ystd ** 2)
                * (
                    (0.5 * np.arange(self.full_res[1] + 1) / (self.full_res[1] + 1))
                    ** 2
                )
            )
        else:
    ...
    def _re_mask(self,):
        pmins = [
            [0.81793009, -1.55666629, 0.03879234, 0.02664923],
        ...
        pmaxs = [
            [-0.76916621, 1.8524333, -1.43464388, 0.34984857],
        ...
        boa_mask = np.all(self.boa >= 0.001, axis=0) & np.all(self.boa < 1, axis=0)
        toa_mask = ~self.bad_pix[self.hx, self.hy]
...
def test_s2():
    ...
    sensor_sat = "MSI", "S2A"
    base = (
        "/data/nemesis/tmp/S2A_MSIL1C_20171201T094341_N0206_R036_T34TDT_20171201T114354.SAFE/"
    ...
    view_angles = [
        ang_base + i + ".tif" for i in ["B02", "B03", "B04", "B08", "B11", "B12"]
    ]
    sun_angles = ang_base.replace("VAA_VZA_", "SAA_SZA.tif")
    obs_time = datetime(2017, 12, 1, 9, 45, 19)
    cloud_mask = (
        gdal.Open(
            base.replace("IMG_DATA/T34TDT_20171201T094341_", "cloud.tif")
        ).ReadAsArray()
        / 100.0
    )
...
def test_l8():
    ...
    view_angles = [
        base + "sensor_%s" % i + ".tif"
        for i in ["B02", "B03", "B04", "B05", "B06", "B07"]
    ]
...
def test_modis():
    ...
    modis_l1b = MODIS_L1b_reader(
        "/data/selene/ucfajlg/Ujia/MODIS_L1b/GRIDDED/", "h17v05", 2017
    )
    ...
    sza = gdal.Open(a.sza).ReadAsArray() / 100.0
    ...
    for i in range(7):
        ...
        band_name = (
            "/MODIS_"
            + tile
            + obs_time.strftime("_%Y%m%d_%H%M_")
            + "band%d.tif" % (i + 1)
        )
