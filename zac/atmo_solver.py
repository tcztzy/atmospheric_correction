...
class solving_atmo_paras(object):
    ...
    def __init__(
        ...
        self.boa = boa
        self.toa = toa
        self.sza = np.cos(sza * np.pi / 180.0)
        self.vza = np.cos(vza * np.pi / 180.0)
        self.saa = np.cos(saa * np.pi / 180.0)
        self.vaa = np.cos(vaa * np.pi / 180.0)
        if self.sza.ndim == 3:
            self.sza, self.saa = self.sza[0], self.saa[0]
        self.raa = np.cos((self.saa - self.vaa) * np.pi / 180.0)
        ...
        self.gamma = gamma
        self.alpha = alpha
        self.band_weights = (np.array(band_wavelength) / 1000.0) ** self.alpha
    ...
    def _grid_conversion(self, array, new_shape):
        ...
        nx, ny = new_shape
        nx, ny = 1.0 * np.arange(nx) / nx * rs, 1.0 * np.arange(ny) / ny * cs
        znew = f(nx, ny)
        return znew

    def _base_grid_resample(self, array):
        rs, cs = array.shape
        self.resample_hx = (1.0 * self.Hx / self.full_res[0] * rs).astype(int)
        self.resample_hy = (1.0 * self.Hy / self.full_res[1] * cs).astype(int)
    ...
    def _multi_grid_solver(self,):
        ...
        self.logger.propagate = False
        bx, by = np.ceil(1.0 * np.array(self.full_res) * self.pix_res / self.aero_res)
        level_x = math.log(bx, 2)
        level_y = math.log(by, 2)
        level = int(min(level_x, level_y))
        scale_factors = 1.0 / 2 ** np.arange(level)[::-1]
        ...
        for _, ii in enumerate(order):
            ...
            self.logger.info(
                bcolors.RED + "Optimizing at grid level %d" % (ii + 1) + bcolors.ENDC
            )
            ...
            if _ == 0:
                ...
                self.aot, self.tcwv = (
                    self._grid_conversion(self.aot_prior.copy(), shape),
                    self._grid_conversion(self.tcwv_prior.copy(), shape),
                )
            if self.vza.ndim == 2:
                ...
                for i, parameter in enumerate(same_var):
                    self.control_variables[
                        :, same_ind[i], :, :
                    ] = self._grid_conversion(parameter, shape)
            ...
            nx, ny = (
                np.ceil(
                    np.array(self.full_res)
                    / np.array([self.num_blocks_x, self.num_blocks_y])
                )
                * np.array([self.num_blocks_x, self.num_blocks_y])
            ).astype(int)
            ...
            self._coarse_num = self._coarse_num.reshape(
                self.num_blocks_x, x_size, self.num_blocks_y, y_size
            ).sum(axis=(1, 3))
            # if ii < 0:
            ...
            self.b_m_pixs = 4.0  # self._coarse_num.max()#(self.aero_res / 500.)**2
            ...
            # psolve = res2
            self.logger.info(
                bcolors.GREEN + str(psolve["message"].decode()) + bcolors.ENDC
            )
            self.logger.info(
                bcolors.GREEN + "Iterations: %d" % int(psolve["nit"]) + bcolors.ENDC
            )
            self.logger.info(
                bcolors.GREEN
            ...
            if ii != len(shapes) - 1:
                mask = (
                    (self.aot_prior <= 0)
                    | (self.aot_prior > 2.0)
                    | (self.tcwv_prior <= self.bot_bounds[1])
        ...
        self.tco3_prior, self.tco3_unc = (
            self._grid_conversion(self.tco3_prior, shape),
            self._grid_conversion(self.tco3_unc, shape),
        )
    ...
    def _helper(self, inp):
        ...
        tmp1 = self._base_grid_resample(tmp1)[..., None]
        tmp2 = np.array(
            [self._base_grid_resample(tmp2[:, :, i]) for i in range(2)]
        ).transpose(1, 0)
        return np.hstack([tmp1, tmp2])
    ...
    def _obs_cost_test(self, p, is_full=True, do_unc=False):
        ...
        xap_dH, xbp_dH, xcp_dH = ret[:, :, 1:].reshape(
            3, self.boa.shape[0], len(self.Hx), 2
        )
        y = xap_H * self.toa - xbp_H
        sur_ref = y / (1 + xcp_H * y)
        diff = sur_ref - self.boa
        full_J = np.nansum(
            0.5 * self.band_weights[..., None] * (diff) ** 2 / self.boa_unc ** 2, axis=0
        )
        J = np.zeros(self.full_res)
        dH = (
            -1
            * (
                -self.toa[..., None] * xap_dH
            ...
            / (
                ...
                - xbp_H[..., None] * xcp_H[..., None]
                + 1
            )
            ** 2
        )
        full_dJ = [
            self.band_weights[..., None] * dH[:, :, i] * diff / (self.boa_unc ** 2)
            for i in range(2)
        ]
        ...
        if is_full:
            ...
            if do_unc:
                ...
                self.obs_unc[:] = np.nan
                self.obs_unc[:, self.Hx, self.Hy] = comb_unc
                self.obs_unc = self.obs_unc.reshape(
                    2, self.num_blocks_x, x_size, self.num_blocks_y, y_size
                )
    ...
    def _smooth_cost(self, p, is_full=True):
        ...
        J_aot, J_aot_ = self._fit_smoothness(aot, 1.0 / self.gamma)
        J_tcwv, J_tcwv_ = self._fit_smoothness(tcwv, 1.0 / self.gamma)
